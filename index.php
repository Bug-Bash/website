...
{
    Session::set_alert(['msg' => $msg, 'type' => $type]);
}
...
app\get('/add', function($req) {
    $data = ['bashes'=>BugBash::listing(), 'page-title'=>"Add New Bug Bash | "];
    return template\compose("add.html", compact('data'), "layout.html");
});

app\post('/add', function($req) {
    $response = BugBash::add($req['form']);
