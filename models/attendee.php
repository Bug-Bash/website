...
class Attendee {
    ...
    {
        $mandatory_fields = array('bash_id'=>'Bash Id', 'participant_name'=>'Participant Name', 'email'=>'Email', 'org'=>'Organisation');
        ...
        $response = mysql\row("select count(*) as `count` from attendees where bash_id ='%s' and email='%s'", array($form['bash_id'], $form['email']));
        if(isset($response['count']) and $response['count']>0)
            return array('status'=>'danger', 'msg'=>'You have already registered for this bash!');
        mysql\query("INSERT INTO attendees (bash_id, participant_name, email, org) VALUES ('%s', '%s', '%s', '%s')", array($form['bash_id'], $form['participant_name'], $form['email'], $form['org']));
